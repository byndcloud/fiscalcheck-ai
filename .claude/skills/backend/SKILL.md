...
description: >
  ...
  arquivo em apps/api/src/fiscocheck_api/. Use também quando o usuário falar em
...
> **`references/00-fiscocheck-context.md`** — princípios não-negociáveis, estrutura de pastas, comandos `uv`, arquivos críticos. Leia antes de qualquer outra referência desta skill.
...
- Estrutura por módulo: cada pasta em [`apps/api/src/fiscocheck_api/modules/`](../../../apps/api/src/fiscocheck_api/modules/) tem `router.py`, `schemas.py`, `service.py`, `models.py`, `repository.py` (`__init__.py` para expor).
...
- Antes de enviar qualquer dado identificável a um LLM externo, **pseudonimize** com `pseudonymize()` de [`core/security.py`](../../../apps/api/src/fiscocheck_api/core/security.py).
