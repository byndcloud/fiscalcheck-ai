...
| Camada | Decisão | Onde |
|---|---|---|
| Web framework | **FastAPI** 0.115 | [`main.py`](../../../../apps/api/src/fiscocheck_api/main.py) |
| Schemas | **Pydantic v2** + `pydantic-settings` | [`core/config.py`](../../../../apps/api/src/fiscocheck_api/core/config.py) |
| ORM | **SQLAlchemy 2.0** com sintaxe nova (`Mapped[]`, `mapped_column`, `select()`) | [`db/base.py`](../../../../apps/api/src/fiscocheck_api/db/base.py) |
| Driver | **asyncpg** (app async) + **psycopg** (Alembic sync) | [`db/session.py`](../../../../apps/api/src/fiscocheck_api/db/session.py) |
...
| Logging | **structlog** estruturado JSON | [`core/logging.py`](../../../../apps/api/src/fiscocheck_api/core/logging.py) |
| Auth | bcrypt (passlib) + JWT (python-jose) + pyotp (MFA) | [`core/security.py`](../../../../apps/api/src/fiscocheck_api/core/security.py) |
...
```powershell
pnpm --filter @fiscocheck/api dev       # uvicorn
pnpm --filter @fiscocheck/api lint
pnpm --filter @fiscocheck/api typecheck
pnpm --filter @fiscocheck/api test
pnpm --filter @fiscocheck/api migrate   # alembic upgrade head
```

## 5. Configuração e segredos

`Settings` ([`core/config.py`](../../../../apps/api/src/fiscocheck_api/core/config.py)) lê do `.env` e do ambiente. No Replit, **Secrets** injetam as env vars (não use `.env` físico em produção).
...
[`core/logging.py`](../../../../apps/api/src/fiscocheck_api/core/logging.py) define `CorrelationIdMiddleware`: a cada requisição, o header `X-Correlation-Id` é (a) lido se o cliente enviar válido, (b) gerado UUIDv4 senão, (c) propagado para todos os logs estruturados via `contextvars`, e (d) devolvido na resposta. Toda chamada ao banco / a serviços externos deve incluí-lo nos logs.

O frontend já propaga via [`apps/web/lib/api-client.ts`](../../../../apps/web/lib/api-client.ts).

## 7. Pseudonimização — `pseudonymize()`

[`core/security.py`](../../../../apps/api/src/fiscocheck_api/core/security.py) — HMAC-SHA256 com `PSEUDONYMIZATION_SALT`. Propriedades:
...
| Path | Por quê |
|---|---|
| [`core/security.py`](../../../../apps/api/src/fiscocheck_api/core/security.py) | Auth + pseudonimização. Bug = vazamento sistêmico. |
| [`core/config.py`](../../../../apps/api/src/fiscocheck_api/core/config.py) | Segredos, validator de placeholders. |
| [`core/logging.py`](../../../../apps/api/src/fiscocheck_api/core/logging.py) | Cadeia de custódia. |
...
## 9. Anti-padrões específicos do FiscoCheck (lista curta)
